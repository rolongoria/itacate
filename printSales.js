import { panelaAndPancakesParentMenuSelection } from "./itemModifierProducts.js";
    ...
    if (menuItemColumn.includes(itemObject[key]["Menu Item"])) {
      ...
      if (itemObject[key]["Menu Item"] == "PLATE 9 - ATACATE ORIGINAL") {
        ...
        if (itemSelection.hasOwnProperty("PUPUSA ORIGINAL")) {
          ...
          itemSelection["PUPUSA ORIGINAL"] +=
            1 * Number(itemObject[key]["Qty"]);
        }
        ...
        if (itemSelection.hasOwnProperty("GORDITA ORIGINAL (PORK)")) {
          // itemSelection[itemObject[key]['Menu Item']] += Number(itemObject[key]['Qty'])
          itemSelection["GORDITA ORIGINAL (PORK)"] +=
            1 * Number(itemObject[key]["Qty"]);
        }
        // if the product is not in the plate object, create it and initialize it with the current Qty value
        else {
      ...
      //EVERY OTHER ITEM
        ...
        if (itemSelection.hasOwnProperty(itemObject[key]["Menu Item"])) {
          // itemSelection[itemObject[key]['Menu Item']] += Number(itemObject[key]['Qty'])
          itemSelection[itemObject[key]["Menu Item"]] +=
            Number(itemObject[key]["Qty"]) *
            itemSelectionMultiplier[itemObject[key]["Menu Item"]];
        }
        // if the product is not in the plate object, create it and initialize it with the current Qty value
        else {
          // itemSelection[itemObject[key]['Menu Item']] = Number(itemObject[key]['Qty'])
...
export function printItemModifiers(
  itemObject,
  productsObject,
  modifierColumn,
  parentMenuSelectionColumn,
) {
  ...
  // console.log(itemObject);
  for (const key in itemObject) {
    // console.log(parentMenuSelectionColumn);
    // console.log(itemObject[key]);
    // console.log(itemObject[key]["Parent Menu Selection"]);
    if (
      parentMenuSelectionColumn.includes(
        itemObject[key]["Parent Menu Selection"],
      )
    ) {
      // console.log(itemObject[key]["Parent Menu Selection"]);
      // console.log(itemObject[key]["Modifier"]);
      // Check if the current itemModifier is in the itemModifier Array (Modifiers that we are interested to count)

      if (
        modifierColumn[itemObject[key]["Parent Menu Selection"]].includes(
        ...
        // ATACATE
        if (
          itemObject[key]["Parent Menu Selection"] == "PLATE 9 - ATACATE CUST"
        ) {
          if (itemModifier.hasOwnProperty(itemObject[key]["Modifier"])) {
            itemModifier[itemObject[key]["Modifier"]] += Number(
              itemObject[key]["Qty"],
            );
        ...
        // TABLA FOR 4 /////////////////////////////////////////
          ...
          if (itemModifier.hasOwnProperty("PUPUSAS Original / Pork")) {
            ...
            itemModifier["PUPUSAS Original / Pork"] +=
              1 * Number(itemObject[key]["Qty"]);
            // console.log(itemModifier)
          }
          // if the modifier is not in the object, create it and initialize it with the current Qty value
          ...
          if (itemModifier.hasOwnProperty("PUPUSAS Rajas Poblanas")) {
            itemModifier["PUPUSAS Rajas Poblanas"] +=
              1 * Number(itemObject[key]["Qty"]);
            // console.log(itemModifier)
          }
          // if the modifier is not in the object, create it and initialize it with the current Qty value
          ...
          if (
            (itemObject[key]["Modifier"] == "Chicharron VERDE") |
            (itemObject[key]["Modifier"] == "Chicharron ROJO")
          ) {
          ...
          if (
            (itemObject[key]["Modifier"] == "Panela en salsa VERDE") |
            (itemObject[key]["Modifier"] == "Panela") |
            (itemObject[key]["Modifier"] == "Panela en salsa ROJA")
          ) {
        ...
        // MEDIA ORDEN DE FLAUTAS
          ...
          if (
            itemModifier.hasOwnProperty(
              productsObject[itemObject[key]["Parent Menu Selection"]] +
                " " +
                itemObject[key]["Modifier"],
            )
          ) {
        ...
        /////// QUESO PANELA AND PANCAKES
        //Revisa si el array panelaParentSelection incluye el plato actual.
        else if (
          panelaAndPancakesParentMenuSelection.includes(
            itemObject[key]["Parent Menu Selection"],
          )
        ) {
          if (
            itemObject[key]["Modifier"] == "Panela Asado" ||
            itemObject[key]["Modifier"] == "Side Panela" ||
            itemObject[key]["Modifier"] == "SD Panela" ||
            itemObject[key]["Modifier"] == "ADD Panela SD" ||
